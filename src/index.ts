...
function getDefaultBoardId() {
  return process.env.TRELLO_DEFAULT_BOARD_ID;
}

function resolveBoardId(boardId?: string) {
  const resolvedBoardId = boardId || getDefaultBoardId();

  if (!resolvedBoardId) {
    throw new TrelloError(
      "Provide boardId or set TRELLO_DEFAULT_BOARD_ID in the environment.",
    );
  }
